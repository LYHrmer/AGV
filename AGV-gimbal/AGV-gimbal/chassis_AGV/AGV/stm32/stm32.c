...
//	}
  ...
  rudder_PID->rudder_in.S_N + (rudder_PID->stm32_PID_param.rtb_Sum1 * rudder_PID->rudder_in.S_P + rudder_PID->stm32_PID_param.Integrator_d);
	...
	if(chassis_move.chassis_mode_CANsend == 30000)
	{
	if(rudder_PID->rudder_out.Out1>=15000.f) rudder_PID->rudder_out.Out1=15000.f;
	else if(rudder_PID->rudder_out.Out1<=-15000.f) rudder_PID->rudder_out.Out1=-15000.f;
	}	
	else
	{	
