...
void chassis_task(void const *pvParameters)
{
    ...
    if (can_comm_task_init_finish())
    {
        ...
        while (1)
        {
            ...
            {
                // 发送控制数据
               can_comm_board(chassis_move.vx_set, chassis_move.vy_set, chassis_move.chassis_behaviour,(int16_t)chassis_move.chassis_RC->key.v & KEY_PRESSED_OFFSET_CTRL);
							// can_cmd_pitch((int16_t)(gimbal_control.PITCH_.relative_angle*100),(int16_t)(gimbal_control.PITCH_.absolute_angle*100),(int16_t)chassis_move.chassis_RC->key.v & KEY_PRESSED_OFFSET_CTRL);
						}
            // 系统延时
...
static void chassis_feedback_update(chassis_move_t *chassis_move_feedback_update)
{
    chassis_move_feedback_update->yaw_relative_ecd = chassis_move_feedback_update->chassis_yaw_motor->yaw_motor.ecd - chassis_move_feedback_update->chassis_yaw_motor->yaw_motor.Encoder_Offset;
...
static void chassis_set_mode(chassis_move_t *chassis_move_mode)
{
    ...
    // 遥控器设置模式
    if (switch_is_down(chassis_move_mode->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL])&&switch_is_down(chassis_move_mode->chassis_RC->rc.s[1]))
    {
        // 遥控器斗都拨到下侧挡位为底盘无力模式
        chassis_move_mode->chassis_behaviour = CHASSIS_ZERO_FORCE;
    }
    if (switch_is_mid(chassis_move_mode->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL]) || switch_is_up(chassis_move_mode->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL]))
    {
        if (mode == RC_Control)
        {
            // 遥控器中挡以及上档为底盘有力模式
