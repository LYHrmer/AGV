...
void gimbal_behaviour_mode_set(gimbal_control_t *gimbal_mode_set)
{
    ...
    if (gimbal_behaviour == GIMBAL_ZERO_FORCE)
    {
        ...
        gimbal_mode_set->YAW_.gimbal_motor_mode = GIMBAL_MOTOR_RAW;
        gimbal_mode_set->PITCH_.gimbal_motor_mode = GIMBAL_MOTOR_RAW;
    }
    else if (gimbal_behaviour == GIMBAL_RC)
    {
        gimbal_mode_set->YAW_.gimbal_motor_mode = GIMBAL_MOTOR_GYRO;   // yaw轴通过陀螺仪的绝对角控制
        gimbal_mode_set->PITCH_.gimbal_motor_mode = GIMBAL_MOTOR_GYRO; // pitch轴通过陀螺仪的绝对角控制
    }
    ...
    {
        gimbal_mode_set->YAW_.gimbal_motor_mode = GIMBAL_MOTOR_ENCONDE;
        gimbal_mode_set->PITCH_.gimbal_motor_mode = GIMBAL_MOTOR_ENCONDE;
    }
...
static void gimbal_behavour_set(gimbal_control_t *gimbal_mode_set)
{
    ...
    if (gimbal_behaviour == GIMBAL_INIT)
    {
        ...
        else
        {
            init_time++; // 初始化时间增加
            // 是否初始化完成
            if ((fabs(gimbal_mode_set->PITCH_.absolute_angle - INIT_PITCH_SET) > GIMBAL_INIT_ANGLE_ERROR) ||
                (fabs(gimbal_mode_set->YAW_.yaw_motor.ecd - gimbal_mode_set->YAW_.yaw_motor.Encoder_Offset) * MOTOR_ECD_TO_RAD > GIMBAL_INIT_ANGLE_ERROR))
            {
                // 初始化未完成，判断初始化时间
            ...
            else
            {
                ...
                {
                    // 保留数据 -- 记录场地正方向
                    gimbal_mode_set->yaw_positive_direction = gimbal_mode_set->YAW_.absolute_angle_set;
                }
            }
        }
    }
...
//		*yaw = yaw_channel_RC * YAW_RC_SEN - gimbal_control_set->gimbal_rc_ctrl->mouse.x * YAW_MOUSE_SEN * 0.8f;
//		*pitch = -pitch_channel_RC * PITCH_RC_SEN + gimbal_control_set->gimbal_rc_ctrl->mouse.y * PITCH_MOUSE_SEN * 0.8f;
...
static void gimbal_auto_attack_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set)
{
    ...
    // 计算过去设定角度与当前角度之间的差值
    yaw_error = gimbal_control_set->YAW_.absolute_angle_set - gimbal_control_set->YAW_.absolute_angle;
    pitch_error = gimbal_control_set->PITCH_.absolute_angle_set - gimbal_control_set->PITCH_.absolute_angle;
    //  获取上位机视觉数据
    ...
    {
        *yaw = yaw_set_angle - gimbal_control_set->YAW_.absolute_angle - yaw_error;
        *pitch = pitch_set_angle - gimbal_control_set->PITCH_.absolute_angle - pitch_error;
    }
}
...
static void gimbal_auto_scan_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set)
{
    ...
    // 赋值增量
    *yaw = gimbal_control_set->gimbal_auto_scan.yaw_auto_scan_first_order_filter.out - gimbal_control_set->YAW_.absolute_angle - yaw_error;
    *pitch = gimbal_control_set->gimbal_auto_scan.pitch_auto_scan_first_order_filter.out - gimbal_control_set->PITCH_.absolute_angle - pitch_error;
}
...
static void gimbal_init_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set)
{
    ...
    //    }
    // 初始化状态控制量计算
    if (fabs(INIT_PITCH_SET - gimbal_control_set->PITCH_.absolute_angle) > GIMBAL_INIT_ANGLE_ERROR)
    {
        *pitch = (INIT_PITCH_SET - gimbal_control_set->PITCH_.absolute_angle) * GIMBAL_INIT_PITCH_SPEED;
        *yaw = 0.0f;
    }
    else
    {
        *pitch = (INIT_PITCH_SET - gimbal_control_set->PITCH_.absolute_angle) * GIMBAL_INIT_PITCH_SPEED;
        *yaw = (INIT_YAW_SET - gimbal_control_set->YAW_.relative_angle) * GIMBAL_INIT_YAW_SPEED;
    }
}
