...
static void vision_set_target_armor_color(vision_control_t* set_detect_color) 
{
    ...
    if (pos_date.robot_id > ROBOT_RED_AND_BLUE_DIVIDE_VALUE)
    {
        //自己为蓝色
...
//        {
//            //筛选不合理的数据
//            if (bullet_speed >= MIN_SET_BULLET_SPEED && calc_cur_bullet_speed->shoot_data_point->bullet_speed <= MAX_SET_BULLET_SPEED)
//            {
//                //修正弹速 -- 修正精度到整数
...
//    }
	  //筛选不合理的数据
		if (shoot_date.bullet_speed >= MIN_SET_BULLET_SPEED && shoot_date.bullet_speed <= MAX_SET_BULLET_SPEED)
		{
				//修正弹速 -- 修正精度到整数
				shoot_date.bullet_speed = (int16_t)shoot_date.bullet_speed;
		}
}
...
static void get_robot_cur_pos(vision_control_t* robot_pos)
{
    robot_pos->auto_move.cur_pos.x = pos_date.x;
    robot_pos->auto_move.cur_pos.y = pos_date.y;
    robot_pos->auto_move.cur_pos.z = pos_date.angle;
