...
//遥控输入判断
int Remote_Control_Judge(chassis_move_t *chassis_move_control);
...
/** 
  * @brief          遥控输入判断
  * @author         LYH 1.0
  * @retval         判断是否有输入值，如果没有保持上次状态
  */
int Remote_Control_Judge(chassis_move_t *chassis_move_control)
{
	if(chassis_move_control->vx==0&&chassis_move_control->vy==0&&chassis_move_control->wz==0)
	return 1;
	else
	return 0;
}
